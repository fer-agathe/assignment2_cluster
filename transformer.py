...
class MultiHeadedAttention(nn.Module):
    def __init__(self, head_size, num_heads, sequence_length=256):
        ...
        # TODO: Write your code here
        # ==========================
        self.linear_q = torch.nn.Linear(head_size * num_heads, head_size * num_heads)
        self.linear_k = torch.nn.Linear(head_size * num_heads, head_size * num_heads)
        self.linear_v = torch.nn.Linear(head_size * num_heads, head_size * num_heads)
        self.linear_o = torch.nn.Linear(head_size * num_heads, head_size * num_heads)
    ...
    def get_attention_weights(self, queries, keys, mask=None):
        ...
        # TODO: Write your code here
        # ==========================
        # Dot product of the last two dimensions of matrices queries and keys
        product = torch.matmul(queries, keys.transpose(-2, -1))
        product = product/self.head_size**0.5

        if mask is not None:
            mask = mask.unsqueeze(1).unsqueeze(1)
            product = product.masked_fill(mask == 0, float('-inf')) # -inf to have probability of 0 where mask == True
    ...
    def split_heads(self, tensor):
        ...
        output = tensor.view(batch_size, self.sequence_length, self.num_heads, self.head_size)
    ...
    def merge_heads(self, tensor):
        ...
        output = output.view(batch_size, self.sequence_length, -1)
        return output
    ...
    def forward(self, hidden_states, mask=None):
        ...
        # Self attention
        attention = self.apply_attention(queries, keys, values, mask)
...
class PreNormAttentionBlock(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        attention_outputs = self.attn(normalized_x, mask) + x
        normalized_attention_outputs = self.layer_norm_2(attention_outputs)
        outputs = self.linear(normalized_attention_outputs)
...
class Transformer(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        # Add CLS token and positional encoding : at the beginning of each sequence in a batch B
        cls_token = self.cls_token.repeat(B, 1, 1)
        x = torch.cat([cls_token, x], dim=1) # add first token to the beginning of sequence
        ...
        # Update mask
        if not mask is None:
            # create mask for cls token
            cls_token_mask = torch.zeros(B, 1) # mask of 1 at each beginning of sequence of batch B
